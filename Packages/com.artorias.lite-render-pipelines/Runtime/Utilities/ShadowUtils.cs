...
namespace LiteRP
{
    ...
    public static class ShadowUtils
    {
        ...
        public static Vector4 GeMainLightShadowBias(ref VisibleLight shadowLight, Vector4 mainLightShadowBias, bool supportsSoftShadows, Matrix4x4 projectionMatrix, int resolution)
        {
            float frustumSize;
            if (shadowLight.lightType == LightType.Directional)
            {
                // Frustum size is guaranteed to be a cube as we wrap shadow frustum around a sphere
                frustumSize = 2.0f / projectionMatrix.m00;
            }
            else
            {
            ...
            {
                SoftShadowQuality softShadowQuality = SoftShadowQuality.UsePipelineSettings;
                // TODO : AdditionalLightData
        ...
        private static bool ExtractDirectionalLightMatrix(ref CullingResults cullResults, ShadowData shadowData, int shadowLightIndex, int cascadeIndex, int shadowmapWidth, int shadowmapHeight, int shadowResolution, float shadowNearPlane, out Vector4 cascadeSplitDistance, out ShadowSliceData shadowSliceData)
        {
            bool success = cullResults.ComputeDirectionalShadowMatricesAndCullingPrimitives(shadowLightIndex,
                cascadeIndex, shadowData.m_MainLightShadowCascadesCount, shadowData.m_MainLightShadowCascadesSplit, shadowResolution, shadowNearPlane, out shadowSliceData.viewMatrix, out shadowSliceData.projectionMatrix,
        ...
        private static void ApplySliceTransform(ref ShadowSliceData shadowSliceData, int atlasWidth, int atlasHeight)
        {
